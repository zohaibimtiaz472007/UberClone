...
const ProfileTab = ({ stats }) => {
  ...
  const driver = {
    ...
    captainId: captain?._id || "CAP123456",
    email: captain?.email || "email@example.com",
    ...
    vehicle: {
      model:
        captain?.vehicle?.vehicleType?.charAt(0).toUpperCase() +
          captain?.vehicle?.vehicleType?.slice(1) || "Vehicle",
      registration: captain?.vehicle?.plate || "N/A",
      color: captain?.vehicle?.color || "N/A",
      year: "2023", // Assuming default or not in schema
    },
    documents: {
      license: "Verified",
      ...
      <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-6">
        ...
        <div className="space-y-3">
          ...
          <ProfileInfoRow
            ...
            value={driver.vehicle.registration}
          />
      ...
      <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-6">
        ...
        <div className="space-y-3">
          <DocumentRow
            ...
            status={driver.documents.license}
          />
          <DocumentRow
            document="Vehicle Registration"
            status={driver.documents.registration}
          />
          <DocumentRow
            document="Insurance"
            status={driver.documents.insurance}
          />
          <DocumentRow
            document="Pollution Certificate"
            status={driver.documents.pollution}
          />
        </div>
      </div>
...
const DocumentRow = ({ document, status }) => {
  ...
  return (
    <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl hover:bg-white/10 transition-all">
      ...
      <div className="flex items-center gap-2">
        <span
          className={`text-sm ${isVerified ? "text-green-400" : "text-red-400"}`}
        >
          {status}
        </span>
        {isVerified && (
